...
def generateSignature(params, key):
    if not isinstance(params, (dict, str)):
        raise Exception(f"string or dict expected, {str(type(params))} given")
...
def generateRandomString(length):
    chars = ''.join(secrets.choice(string.ascii_uppercase + string.digits + string.ascii_lowercase) for _ in range(length))
    return chars
